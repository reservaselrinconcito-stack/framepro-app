...
export const PageBar: React.FC<PageBarProps> = ({
    ...
    function addCustomPage() {
        ...
        onAddPage({ slug, label: 'Nueva página' });
                    ...
                    <div key={slug} className="relative flex items-center shrink-0">
                        {isRenaming ? (
                            ...
                            <button
                                ...
                                className={`flex items-center gap-1.5 h-7 px-3 rounded-lg text-[10px] font-black transition-all whitespace-nowrap ${
                                    isActive
                ...
                {showAddMenu && (
                    <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden min-w-[160px]">
                        {availablePresets.map(p => (
                            <button
                                key={p.slug}
                                onClick={() => { onAddPage({ slug: p.slug, label: p.label }); setShowAddMenu(false); }}
