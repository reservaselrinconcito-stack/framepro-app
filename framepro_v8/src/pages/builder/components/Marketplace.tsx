...
interface MarketplaceProps {
    onUseTemplate: (config: SiteConfigV1, templateId: string) => void;
    onClose: () => void;
}
                                    ...
                                    {images.map((img, i) => (
                                        <button
                                            ...
                                            className={`w-16 h-12 rounded-xl overflow-hidden border-2 transition-all ${
                                                activeImg === i ? 'border-indigo-500' : 'border-slate-200'
                                            }`}
                                        >
...
export const Marketplace: React.FC<MarketplaceProps> = ({ onUseTemplate, onClose }) => {
    ...
    const handleUseTemplate = useCallback(async (template: FrameProTemplate) => {
        ...
        if (template.isFree || template.price === 0) {
            const config = demoMode.createFromTemplate(template.id, template.config.globalData.brandName);
            onUseTemplate(config, template.id);
