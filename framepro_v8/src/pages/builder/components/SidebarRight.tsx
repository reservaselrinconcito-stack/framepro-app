        ...
        <aside className="w-80 bg-white border-l border-slate-200 flex flex-col h-full z-40 shrink-0 select-none shadow-sm animate-in slide-in-from-right duration-300">
            ...
            <div className="p-2 border-b border-slate-100 flex gap-1 bg-slate-50/30">
                ...
                <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-2 rounded-xl text-[10px] font-black bg-gradient-to-r from-violet-500 to-indigo-500 text-white hover:from-violet-600 hover:to-indigo-600 transition-all shadow-md shadow-violet-100"
            ...
            {showAI && selectedBlock && (
                <AIContentGenerator
                    block={selectedBlock}
                    businessContext={businessContext}
                    onApply={(id, data) => onUpdateBlock(id, { data })}
            ...
            <div className="grid grid-cols-2 gap-1.5">
                {variants.map(v => (
                    <button
                        ...
                        title={v.description}
                        className={`flex items-center gap-2 p-2.5 rounded-xl border-2 text-left transition-all group ${
                            currentVariant === v.id
