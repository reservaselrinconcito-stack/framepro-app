...
import { Marketplace } from './builder/components/Marketplace';
...
// Optional: RentikPro integration (graceful degradation if not available)
let projectManager: any = null;
let publishAdapter: any = null;
let generateV0Config: any = null;
let APP_VERSION = '1.0.0';

try {
    // These imports may fail in standalone FramePro mode
    projectManager = require('@/services/projectManager').projectManager;
    publishAdapter = require('../services/publishAdapter').publishAdapter;
    generateV0Config = require('../modules/webBuilder/v0Generator').generateV0Config;
    APP_VERSION = require('../version').APP_VERSION;
} catch {
    // Running in standalone FramePro mode (demo / no backend)
    demoMode.enable();
}

const IS_DEMO = demoMode.isActive();

// ─── Error Boundary ────────────────────────────────────────────────────────────

class BuilderErrorBoundary extends Component<{ children: React.ReactNode }, { hasError: boolean; error?: Error }> {
...
function useVersionChecker() {
    useEffect(() => {
        if (import.meta.env.DEV || IS_DEMO) return;
...
const FrameProEditorInner: React.FC = () => {
    ...
    async function loadSites() {
        ...
        // Demo sites
        const demoProjects = demoMode.listProjects();
    ...
    function handleDuplicate(site: SiteItem) {
        const newConfig = demoMode.duplicateProject(site.id);
    ...
    function handleImportJson(json: string): boolean {
        ...
        demoMode.saveProject(config, 'custom');
        const site: SiteItem = {
            id: config.slug,
    ...
    const handleSave = useCallback(async (isAuto = false) => {
        ...
        try {
            if (selectedSite.source === 'demo' || IS_DEMO) {
                demoMode.saveProject(config, selectedSite.templateId ?? 'custom');
            } else if (projectManager) {
            ...
            <div className="min-h-screen bg-slate-50 font-sans">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                                Editor Web Visual {IS_DEMO && '· Modo Demo'}
                            </p>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                            {sites.map(site => (
                                <SiteCard
                                    ...
                                    onDelete={() => {
                                        if (site.source === 'demo') {
                                            demoMode.deleteProject(site.id);
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                            {FRAMEPRO_TEMPLATES.map(t => (
                                <button
                                    key={t.id}
                                    onClick={() => {
                                        const config = demoMode.createFromTemplate(t.id, t.config.globalData.brandName);
                                        const site: SiteItem = {
                ...
                {showNewSiteModal && (
                    <NewSiteModal
                        ...
                        onCreate={(config, templateId) => {
                            const site: SiteItem = {
                                id: config.slug, name: config.globalData.brandName,
                                slug: config.slug, templateId, source: 'demo', config,
                            };
                            demoMode.saveProject(config, templateId);
                            setSites(s => [...s, site]);
                ...
                {showImportModal && (
                    <ImportModal
                        onImport={(json) => {
                            const ok = handleImportJson(json);
                ...
                {showMarketplace && (
                    <Marketplace
                        onUseTemplate={(config, templateId) => {
                            const site: SiteItem = {
                                id: config.slug, name: config.globalData.brandName,
                                slug: config.slug, templateId, source: 'demo', config,
                            };
        ...
        <div className="h-screen flex flex-col bg-slate-50 overflow-hidden font-sans">
            ...
            <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 z-50 shrink-0">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        {isPublishing ? <Loader2 size={14} className="animate-spin" /> : IS_DEMO ? <Download size={14} /> : <Globe size={14} />}
                        {isPublishing ? 'Publicando…' : IS_DEMO ? 'Exportar HTML' : 'Publicar'}
                    </button>
                </div>
            </header>
...
const NewSiteModal: React.FC<{
    onClose: () => void;
    onCreate: (config: SiteConfigV1, templateId: string) => void;
}> = ({ onClose, onCreate }) => {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative z-[101] bg-white rounded-[2rem] shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                        {FRAMEPRO_TEMPLATES.map(t => (
                            <button
                                ...
                                className={`flex flex-col rounded-2xl overflow-hidden border-2 transition-all hover:scale-105 ${
                ...
                <div className="p-8 border-t border-slate-100 flex gap-3 shrink-0">
                    ...
                    <button
                        onClick={() => {
                            const config = demoMode.createFromTemplate(
                                selectedTemplate.id,
                                brandName.trim() || selectedTemplate.config.globalData.brandName
                            );
                            onCreate(config, selectedTemplate.id);
                        }}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative z-[101] bg-white rounded-[2rem] shadow-2xl w-full max-w-lg p-8">
                ...
                <div
                    ...
                    className={`flex flex-col items-center justify-center gap-3 p-8 rounded-2xl border-2 border-dashed cursor-pointer transition-all mb-4 ${
                        dragging
